...
Yes. But for scripts execution (`forge script ...`) it's recommended to use the `--skip-simulation` flag, as gas logic on EraVM differs from the EVM and foundry can estimate gas incorrectly.
...
You can deploy contracts just like on Ethereum (e.g., a transaction without field `to` and with the contract init code as calldata) and use tools like Hardhat

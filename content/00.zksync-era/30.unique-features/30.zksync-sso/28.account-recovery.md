...
The guide below provides information about the linking and recovery process, with further details about these two main components in the solution:

- An [ERC7579 validator module](https://erc7579.com/modules) (`OidcRecoveryValidator`) is in charge of managing the recovery data of every SSO account.
- A **ZK circuit** is used to prove the ownership of an account without revealing the account

## Process helicopter view
...
```mermaid
sequenceDiagram
  User ->> Google: asks for JWT
  Google -->> User:
  User ->> SaltService: Ask for salt
  SaltService -->> User:
  User ->> User: Calculates OIDC digest
  User ->> OidcRecoveryValidator: Save OIDC digest
```
...
After completing this process, users can add a new passkey to access their SSO account and successfully recovered their SSO account.

```mermaid
sequenceDiagram
  User ->> User: Calculates nonce
 User ->> Google: Asks for JWT <br> (using nonce)
 Google -->> User:
 User ->> User: Retrieves salt
 User ->> User: Generates zk proof
 User ->> OidcRecoveryValidator: sends proof
 OidcRecoveryValidator ->> OidcRecoveryValidator: recreates public input
 OidcRecoveryValidator ->> ZkVerifier: Proof + public input
 ZkVerifier ->> OidcRecoveryValidator: Success
 note over OidcRecoveryValidator: Recovery success
...
is an [ERC7579 validator module](https://erc7579.com/modules) that enables zero-knowledge-based account recovery via OpenID Connect (OIDC)
identity assertions. It enables user to add a new passkey if they can prove control over an OIDC identity
(e.g., their Google account) previously linked to their SSO account.
...
The circuit is implemented in **Circom** and leverages code from `circomlib` and `zkemail`.
...
![image.png](OIDC%20for%20ZKsync%20docs%201f24210eb1b080a8a441eb5d0431acb2/image.png)
...
https://github.com/Moonsong-Labs/zksync-social-login-circuit/

**OidcRecoveryValidator:**

https://github.com/matter-labs/zksync-sso-clave-contracts/blob/main/src/validators/OidcRecoveryValidator.sol

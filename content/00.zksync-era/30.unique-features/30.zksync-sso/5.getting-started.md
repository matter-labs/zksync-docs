...
3. Deploy an account
    ...
    const account = await registerAccount(accountDetails, challenge, config);
    ```

4. Send a transaction

    ```ts
    import type { AccountClient } from 'react-native-zksync-sso';

    const accountClient = new AccountClient(account, rpId, config);

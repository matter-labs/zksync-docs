...
:::info The operator is NOT required to use the actual value and can use any value up to that signed by the user. :::
...
Returns the `msg.sender` of the L1->L2 transaction as the `common.Address` of the contract that initiated the
transaction.
...
:::warning The zkSync Era implementation is slightly different from Ethereum. :::
...
:::tip The `prefix` is equal to `keccak256("zksyncCreate2")`. :::
...
Returns the `msg.sender` of the L2->L1 transaction as the `common.Address` of the contract that initiated the
transaction.

...
The [utilities library](https://github.com/zksync-sdk/zksync-ethers/blob/main/src/utils.ts) contains essential utilities
for building on zkSync Era.
...
Checks if the transaction's base cost is greater than the provided value, which covers the transaction's cost. Throws an
error if it is not.
...
Generates a future-proof contract address using salt plus bytecode which allows determination of an address before
deployment.
...
:::tip The `prefix` is equal to `keccak256("zksyncCreate")`. :::
...
const withdrawETHMessageHash = utils.getHashedL2ToL1Msg(
  '0x36615Cf349d7F6344891B1e7CA7C72883F5dc049',
  withdrawETHMessage,
  0
);
...
Called from [`isSignatureCorrect`](#isSignatureCorrect) for contract account addresses, the function returns true if the
validation process results in the `EIP1271_MAGIC_VALUE`.
...
Called from [`isMessageSignatureCorrect`](#ismessagesignaturecorrect) and
[`isTypedDataSignatureCorrect`](#istypeddatasignaturecorrect). Returns true if account abstraction EIP712 signature is
correct.
...
- `transaction.customData.customSignature` is an empty `string`. The transaction should be signed and the
  `transaction.customData.customSignature` field should be populated with the signature. It should not be specified if

...
1. **Execute calls:** Call a contract on another ZKsync chain or Ethereum L1 with specific calldata and value.
1. **Execute bundles of calls:** Group multiple remote calls into a single bundle, ensuring all of them execute at once.
1. **Control shadow accounts:** Manage smart accounts on other chains, including L1, to execute transactions and manage assets
  without needing a local account on that chain.
...
5. **Bundle Submission**: A relayer submits the bundle together with a message inclusion proof to the receiving ZKsync chain.
  This proof connects the bundle message to the imported interop root.
6. **Verification and Execution**: The receiving ZKsync chain verifies the bundle message against the interop root.
...
1. [Messaging](/zksync-network/zksync-connect/messaging)
1. [Asset transfers](/zksync-network/zksync-connect/crosschain-asset-transfers),
    and [transaction bundles](/zksync-network/zksync-connect/transaction-bundles)
1. Automatic crosschain transactions, L1 interop, and shadow accounts.

Only chains that settle on top of [Gateway](/zksync-protocol/gateway) have access to interop features.
To see which chains use ZKsync Gateway, check the [Elastic Network Chains](/zksync-network/environment) table.

Messaging is already available on all ZKsync chains that use Gateway.
Asset transfers and transaction bundles are currently only available for local testing,
but will be available for mainnet chains as part of the upcoming [`v31` upgrade](https://forum.zknation.io/t/zip-16-v31-interop-bundles-upgrade/979).

...
To provide easy access to all the features of ZKsync chains, the [`zksync-web3-rs`][sdk] Rust SDK was created, which builds
...
## Connecting to a ZKsync chain

To interact with a ZKsync network users need to know the endpoint of the operator node.
In this tutorial, we will be using the `localnet` from [Dockerized L1 - L2 Nodes](/zksync-network/tooling/local-setup/dockerized-l1-l2-nodes).

...
::callout
See also [`undol1tol2alias`](/zksync-network/sdk/js/ethers/api/v6/utilities#undol1tol2alias).
::
...
| Parameter       | Type                                                                   | Description                                    |
| --------------- | ---------------------------------------------------------------------- | ---------------------------------------------- |
| `transaction`   | `Transaction` or [`TransactionRequest`](/zksync-network/sdk/js/ethers/api/v6/types#transactionrequest) | EIP-712 transaction.                           |
| `ethSignature?` | [`EthereumSignature`](/zksync-network/sdk/js/ethers/api/v6/types#ethereumsignature)                    | ECDSA signature of the transaction. |
...
but returns `true` if the validation process succeeds, otherwise returns `false`.
...
Called from [`isSignatureCorrect`](#issignaturecorrect) for contract account addresses,
...
[`isTypedDataSignatureCorrect`](#istypeddatasignaturecorrect).
...
- `transaction.customData.customSignature` is an empty `string`. The transaction should be signed and the
  `transaction.customData.customSignature` field should be populated with the signature.
   It should not be specified if the transaction is not signed.

...
description: Guidelines to help you build secure, efficient, and maintainable smart contracts on ZKsync chains powered by EraVM.
---

Before developing on a ZKsync chain running EraVM, review these best practices. Following these recommendations will
...
ZKsync chains running EraVM also support EVM Bytecode from the `solc` compiler.
::
...
The gas model on ZKsync chains running EraVM differs from Ethereum’s. Two key factors influence gas costs:
...
`anvil-zksync` allows you to simulate mainnet behavior for development purposes.

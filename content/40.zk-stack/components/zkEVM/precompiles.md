...
called with a determined gas cost. The addresses start from `1`, and increment for each contract.

New hard forks may introduce new precompiled contracts. They are called from the opcodes like regular contracts, with
instructions like `CALL`. The gas cost mentioned here is purely the cost of the contract. It does not consider the cost
of the call itself nor the instructions to put the parameters in memory.

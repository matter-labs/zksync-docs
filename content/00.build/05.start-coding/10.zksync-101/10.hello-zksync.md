...
[`contracts/1-hello-zksync` directory][crowdfunding-campaign-sol].

The `CrowdfundingCampaign` contract is designed for a project crowdfunding.
...
[`/deploy/1-deploy-hello-zksync.ts`][deploy-script].
...
We can confirm that by calling the `getFundingGoal` method using ZKsync CLI.

In the terminal, run the following, replacing the contract address with your contract's address:
...
zksync-cli contract write \
--chain in-memory-node \
--contract 0xB9D6120056a5FbbBEDE2016CA30A41c2A43D5aA9 \

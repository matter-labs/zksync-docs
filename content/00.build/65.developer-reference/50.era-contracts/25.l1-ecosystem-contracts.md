...
Since the upgrade to ZK Chains, all hyZK Chainontracts, including ZKsync Era diamond proxy, no longer directly manages any funds.
...
the [L1 smart contracts page](/build/developer-reference/era-contracts/l1-contracts) for a better understanding of single instance ZK Chain.
...
of the entire ZK Chain before its HyZK Chainan get the access to the shared liquidity.
...
| --- | --- | --- |
| chainId | Permanent | Permanent identifier of the ZK Chain. Due to wallet support reasons, for now chainId has to be small (48 bits). This is one of the reasons why for now we’ll deploy HyZK Chainmanually, to prevent HypeZK Chainom having the same chainId as some another popular chain.  In the future it will be trustless-ly assigned as a random 32-byte value.|
...
| admin | By admin of ZK Chain | The governor of the HyZK ChainIt has some limited powers to govern the chain |
...
| executing upgrades | By governance of ZK Chain or STM | While exclusively STM governance can set the content of the upgrade, HyZK Chainwill typically be able to choose suitable time for them to actually execute it. In the first release, HypeZK Chainll have to follow our upgrades.|
...
In case we are aware that some of the committed batches on an ZK Chain are dangerous to be executed, the STM can call `revertBatches`  on that HyZK Chain
...
One of the reasons it was done this way was to have the unified interface for all ZK Chains that will ever be included in the hyZK Chaincosystem.
...
- Once an ZK Chain is created, it will be the job of its first validator (it can be done any user with access to that HyZK Chainto deploy such a bridge.
...
i.e. the there will be no direct ZK Chain<>HyZK Chainransfers supported,

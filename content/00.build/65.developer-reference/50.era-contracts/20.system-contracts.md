...
- After that, it puts the bytecode hash with a
  [special constructing marker](../50.era-contracts/20.system-contracts.md#constructing-vs-non-constructing-code-hash) as code for the address of the
...
- Contains minimal implementation of our account abstraction protocol. Note that it supports the
  [built-in paymaster flows](../40.account-abstraction/50.paymasters.md).

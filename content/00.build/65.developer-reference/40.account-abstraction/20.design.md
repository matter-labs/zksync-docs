...
[NonceHolder](../../zk-stack/components/smart-contracts/system-contracts.md#nonceholder) to check whether the provided nonce has already been used or not.
...
[bootloader](../../zk-stack/components/zkEVM/bootloader.md). Since these
...
This method must send at least `tx.gasprice * tx.gasLimit` ETH to the [bootloader](./system-contracts.md#bootloader) address.
...
interaction with the paymaster. One of the notable [examples](#approval-based-paymaster-flow) where it can be helpful is to approve the ERC-20
...
contracts can only be called with a special `isSystem` flag. You can read more about it [here](./system-contracts.md#protected-access-to-some-of-the-system-contracts).

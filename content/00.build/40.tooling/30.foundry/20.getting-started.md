...
- `--zk-solc-path <ZK_SOLC_PATH>`: Specify a solc binary path to use to compile along zksolc.
- `--zk-enable-eravm-extension <SYSTEM_MODE>`: Enables system contract compilation mode (`true`/`false`).
- `--zk-force-evmla <FORCE_EVMLA>`: Switch to the EVM legacy assembly pipeline.
- `--zk-fallback-oz <FALLBACK_OZ>`: Recompile with `-Oz` if bytecode is too large.
- `--zk-detect-missing-libraries`: Detect and report missing libraries.
- `-O, --zk-optimization <LEVEL>`: Set LLVM optimization levels.

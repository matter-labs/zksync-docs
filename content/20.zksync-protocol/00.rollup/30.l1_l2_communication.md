...
ZKsync supports general message passing for L1<->L2 communication.
Proofs are settled on L1, so core of this process is the [L2->L1] message passing process.
[L1->L2] messages are recorded on L1 inside a priority queue, the sequencer picks it up from here and executes it in the L2.
The L2 sends an L2->L1 message of the L1 transactions that it processed,
...
[opcode](/zksync-protocol/zksync-era/contracts/bootloader).
...
- `txNumberInBatch` the number of the transaction in the batch where the log has happened. This number is taken from the
  ...
  [called](/zksync-protocol/zksync-era/contracts/bootloader).
...
[Batches and L2 blocks on ZKsync](/zksync-protocol/zksync-era/transactions/blocks).
...
Please read [Bootloader](/zksync-protocol/zksync-era/contracts/bootloader) and
[System contracts](/zksync-protocol/zksync-era/contracts/system-contracts)
on the general system contracts / bootloader structure as well as the pubdata structure with Boojum system to understand
[the difference](/zksync-protocol/zksync-era/transactions/fee-model/how-we-charge-for-pubdata)

...
[NonceHolder](/zksync-protocol/era-vm/contracts/system-contracts#nonceholder) to check whether the provided nonce has already been used or not.
...
[bootloader](/zksync-protocol/era-vm/contracts/bootloader). Since these
...
This method must send at least `tx.gasprice * tx.gasLimit` ETH to the [bootloader](/zksync-protocol/era-vm/contracts/bootloader) address.
...
contracts can only be called with a special `isSystem` flag. You can read more about it [here](/zksync-protocol/era-vm/contracts/system-contracts#protected-access-to-some-of-the-system-contracts).

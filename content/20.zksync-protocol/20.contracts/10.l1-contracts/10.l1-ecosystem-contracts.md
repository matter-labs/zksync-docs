...
![Contracts](/images/developer-reference/l1-smart-contracts/zkchain-scheme.png)
...
An ZK Chain does not even have to be an L2 chain, it can also be an L3, etc.
...
This document describes technical details of ZK Stack contracts that make it possible for multiple L2/L3 chains to share liquidity,
...
[ZK Chain](/build/developer-reference/protocol) and read
...
ZK Chain governor can not set its own upgrades and it can only “execute” the upgrade that has already been prepared by the STM.
...
| stateTransitionManager | Permanent | The STM that deployed the ZK Chain. In principle, it could be possible to migrate between STMs (assuming both STMs support that). However, in practice it may be very hard and as of now such functionality is not supported. |
...
| validatorTimelock.validator | STM | The governance of ZK Chain can choose who can submit new batches to the ValidatorTimelock.  |
...
| executing upgrades | By governance of ZK Chain or STM | While exclusively STM governance can set the content of the upgrade, ZK Chain will typically be able to choose suitable time for them to actually execute it. In the first release, ZK Chain have to follow our upgrades.|
...
In case we are aware that some of the committed batches on an ZK Chain are dangerous to be executed, the STM can call `revertBatches`  on that ZK Chain
...
In the previous section we discussed how ZK Chains and STMs work.
...
function createNewChain(
    ...
    // the ZK Chain with the latest protocol version. The correctness of it will be verified by STM.
...
ZKsync ERA will become an ZK Chain, under a newly deployed STM.

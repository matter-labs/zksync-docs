...
These chains are independently deployed rollups or validiums built using the ZK Stack, and together they form a coordinated network secured by Ethereum.

To support this ecosystem, Ethereum requires shared Layer 1 smart contracts. This page outlines the ZK Stack’s approach to these contracts:
...
1. Users tell us (e.g. via a Google Form) that they want to deploy a ZKsync Chain with certain governance,
...
It is also de-facto ZKsync Chain implementation of the ZKsync ERC20 token
...
For better understanding of this section it is recommended to understand (at least roughly) [how bridging on ZKsync Chains works](/zksync-protocol/rollup/bridging-assets).
...
ZKsync Era was the first chain to migrate to the shared bridge and bridge hub and the following steps were required.

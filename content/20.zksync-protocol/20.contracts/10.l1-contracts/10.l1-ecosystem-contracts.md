...
that is focused on an ecosystem of interconnected zkEVMs/zkVMs, (which we name ZKsync Chains).
This ecosystem will be grounded on Ethereum, requiring the appropriate L1 smart contracts.
Here we outline our ZKsync Stack approach for these contracts, their interfaces,
the needed changes to the existing architecture, as well as future features to be implemented.

If you want to know more about ZKsync Chains, check this blogpost on an
[Introduction to ZKsync Chains](https://blog.matter-labs.io/introduction-to-hyperchains-fdb33414ead7),
...
we’ll call each rollup a ZKsync Chain.
Note, that ZKsync Chains can be Validiums.

Since the upgrade to ZKsync Chains, all ZKsync Chain contracts, including ZKsync Era diamond proxy, no longer directly manages any funds.
...
[ZKsync Chain](/zksync-protocol/rollup) and read
...
ZKsync Chain governor can not set its own upgrades and it can only “execute” the upgrade that has already been prepared by the CTM.
...
The exact process of deploying & registering an ZKsync Chain will be described in [sections below](#creating-new-chains-with-bridgehub).
...
| --- | --- | --- |
| chainId | Permanent | Permanent identifier of the ZKsync Chain. Due to wallet support reasons, for now chainId has to be small (48 bits). This is one of the reasons why for now we’ll deploy ZKsync Chain manually, to prevent ZKsync Chain having the same chainId as some another popular chain.  In the future it will be trustless-ly assigned as a random 32-byte value.|
...
| validatorTimelock.validator | CTM | The governance of ZKsync Chain can choose who can submit new batches to the ValidatorTimelock.  |
...
| executing upgrades | By governance of ZKsync Chain or CTM | While exclusively CTM governance can set the content of the upgrade, ZKsync Chain will typically be able to choose suitable time for them to actually execute it. In the first release, ZKsync Chain have to follow our upgrades.|
...
In case we are aware that some of the committed batches on an ZKsync Chain are dangerous to be executed,
the CTM can call `revertBatches` on that ZKsync Chain

### Issues & caveats

- If an ZKsync Chain skips an upgrade
...
In this section we’ll explore how exactly unified liquidity is achieved and how do ZKsync Chains get deployed.
...
In the long run, any `baseToken`  could be used (obviously at the peril of the ZKsync Chain that decides to use it).
...
For this reason, there will be an entity called `admin` which is basically a hot key managed by us and it will be used to deploy new ZKsync Chains.

So the flow for deploying their own ZKsync Chain for users will be the following:

1. Users tell us (e.g. via a Google Form) that they want to deploy an ZKsync Chain with certain governance,
...
3. After the ZKsync Chain validates the tx, it includes it into its priority queue.
...
Upon creation of an ZKsync Chain other chains may want to use their own custom base tokens.
...
Right after an ZKsync Chain is deployed, the `SharedBridge` will already be used for `baseToken`-related bridging.
...
- Once an ZKsync Chain is created, it will be the job of its first validator (it can be done
any user with access to that ZKsync Chain to deploy such a bridge.
...
It is also de-facto ZKsync Chain implementation of the ZKsync Era’s ERC20 token
...
Let’s say that an ZKsync Chain has ETH as its base token.
...
- `secondBridgeValue`  is the `msg.value` to be sent to the bridge which is responsible for the asset being deposited
  ...
  This can be used to deposit ETH to ZKsync Chains that have base token that is not ETH.
...
ZKsync ERA will become an ZKsync Chain, under a newly deployed CTM.
...
but it will be only allowed to be called if the ZKsync Chain is Era.
...
i.e. the there will be no direct ZKsync Chain<>ZKsync Chain transfers supported,

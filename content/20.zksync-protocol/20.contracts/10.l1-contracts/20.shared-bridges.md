...
The Shared Bridge and the associated smart contracts are foundational to the sustainable growth of the ZKsync chain ecosystem.
...
- Acts as a hub for bridges, so that they have a single point of communication with all ZKsync chain contracts. This
  allows L1 assets to be locked in the same contract for all ZKsync chains, including L3s and validiums. The `Bridgehub`

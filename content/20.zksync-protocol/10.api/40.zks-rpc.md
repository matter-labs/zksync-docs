...
Retrieves the bytecode supplier contract.
...
**DATA, 20 bytes** - a single string value representing the bytecode supplier contract address.
...
items: [{
  label: 'curl',
  partial: '_partials/_api/_curl/_zks_get-bytecode-supplier-contract'
}, {
  label: 'TypeScript',
  partial: '_partials/_api/_ts/_zks_get-bytecode-supplier-contract'
}]
---
::
...
{
  "jsonrpc": "2.0",
  "result": "0xc4fd2580c3487bba18d63f50301020132342fdbd",
  "id": 1
}
```
...
Retrieves the metadata for a given block number.

#### Parameters

**QUANTITY** - the number of the block to get the metadata for.

#### Returns

Object containing metadata about the specified block.

- **number**: uint256 - the pubdata price per byte.
- **number**: uint256 - the native price.
- **number**: uint32 - the execution version.
...
items: [{
  label: 'curl',
  partial: '_partials/_api/_curl/_zks_get-block-metadata-by-number'
}, {
  label: 'TypeScript',
  partial: '_partials/_api/_ts/_zks_get-block-metadata-by-number'
}]
---
::
...
```json
{
  "jsonrpc": "2.0",
  "result": {
    "pubdata_price_per_byte":"0x176cb441",
...
**ZKsync Chains Merkle Tree Details:**

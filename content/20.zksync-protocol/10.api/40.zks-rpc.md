...
- **initialContracts**: `Vec<(Address, alloy::primitives::Bytes)>` - Initial contracts to deploy in genesis.
  Storage entries that set the contracts as deployed and preimages will be derived from this field.
- **additionalStorage**: `Vec<(B256, B256)>` - Additional storage entries to add in genesis state (not related to contract deployments).

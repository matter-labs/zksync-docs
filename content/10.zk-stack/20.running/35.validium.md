...
::drop-panel
  ...
  The DA layer receives and stores [L1 batches](https://docs.zksync.io/zksync-protocol/rollup/blocks#the-role-of-l1-batches) from a ZK chain,
  and creates proofs that some specific data is included in a block.

  The specific data being proved depends on the DA layer, but generally includes information such as the block header and transaction data.
...
The `checkDA` function uses a merkle proof to both:

- Check that the output hash of the L2 chain matches the data being validated
- Validate that the data is available on the DA

Separate contracts that implement this interface are deployed for each DA layer.
...
- **Verification bridges:** these are the smart contracts that gets updated with the state roots of the specific DA layers.
  They utilize the ZK proofs to verify the validity of the provided state roots.
  Later on, the verification bridges are called from `L1DAValidator` contract and normally use Merkle-proofs to verify against the known state roots.
...
If we go a step further and do the ZK proof that has the following characteristics:
  ...
  - Verify that the data root is included in the state root.
    ...
    This solution is especially relevant with DA layers that use the commitments that are inefficient to produce on L2.
...
::drop-panel
  ...
  ::panel{label="Configuring for Avail"}
...
2. Add the following block to the `secrets.yaml`:
  ...
  ::panel{label="Configuring for Celestia"}
...
2. Add the following block to the `secrets.yaml`:
  ...
  ::panel{label="Configuring for EigenDA"}
...
so the procedure wouldn't differ a lot.
  ...
  To enable privacy on your chain, you must implement a custom authentication process.

  You can find an [example implementation](https://github.com/Moonsong-Labs/double-zero) of a working private chain created by MoonsongLabs.
  You can also find a [complete tutorial](https://github.com/sarahschwartz/double-zero/blob/main/docs/step-by-step-guide.md)
  for how to use that example to run a game with onchain privacy.

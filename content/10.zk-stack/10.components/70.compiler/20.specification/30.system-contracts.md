...
4. [Revert or throw](exception-handling)
...
1. The [call](instructions/evm/calls) has the Ether value parameter.
...
<!-- TODO: update link -->
<!-- See [handling immutables](/build/developer-reference/differences-with-ethereum.#setimmutable-loadimmutable)
on ZKsync Era documentation. -->
...
Both [zksolc](../toolchain/solidity) and [zkvyper](../toolchain/vyper) compilers for EraVM operate on
[the IR level](../toolchain/overview#ir-compilers), so they cannot control the heap memory allocator which remains a responsibility of
[the high-level source code compilers](../toolchain/overview#high-level-source-code-compilers) emitting the IRs.
...
<!-- TODO: update link - [Returning immutables](/build/developer-reference/differences-with-ethereum#setimmutable-loadimmutable) -->

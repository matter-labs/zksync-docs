...
- Configure contract access policies through the [Prividium™ Admin Dashboard](admin-dashboard), defining which functions are public, private, or role-restricted
...
All interactions with the chain are routed through a [Proxy RPC](proxy) that enforces access policies managed in the [Prividium™ Permissioning System](admin-dashboard).
Administrators configure roles, users, and permissions through the Admin Dashboard, which stores policies dynamically in the [Permissions API](permissions-overview).
**Features:**
...
See [Chain Administration](admin-configuration)

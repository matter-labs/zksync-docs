...
contract InteropAssetTransferSendBundle is Script {
    uint256 internal constant CHAIN2_ID = 6566;
    uint256 internal constant DEFAULT_TRANSFER_AMOUNT = 1_000_000;
    uint256 internal constant MIN_NATIVE_GAS_BUFFER = 0.001 ether;
    ...
    function run(address tokenAddress) external returns (bytes32 bundleHash, bytes32 assetId) {
        ...
        InteropConfig memory config = InteropConfig({
            interopCenterAddress: 0x000000000000000000000000000000000001000d,
            l2NativeTokenVaultAddress: 0x0000000000000000000000000000000000010004,
            l2AssetRouterAddress: 0x0000000000000000000000000000000000010003
        });

        uint256 chain1Fork = vm.createFork(chain1RpcUrl);
        vm.selectFork(chain1Fork);

        uint256 sourceBalanceBefore = IERC20(tokenAddress).balanceOf(sender);
        ...
        require(sourceBalanceBefore >= DEFAULT_TRANSFER_AMOUNT, "insufficient source balance");
        require(
            nativeBalanceBefore >= interopFee + MIN_NATIVE_GAS_BUFFER,
            "insufficient native balance for interop fee and gas"
        );

        vm.startBroadcast(privateKey);
        IERC20(tokenAddress).approve(config.l2NativeTokenVaultAddress, DEFAULT_TRANSFER_AMOUNT);
        (bundleHash, assetId) = _sendErc20Bundle(
            config, CHAIN2_ID, tokenAddress, sender, DEFAULT_TRANSFER_AMOUNT, interopFee
        );
        vm.stopBroadcast();
        ...
        console2.log("Chain 6565 balance before interop:", sourceBalanceBefore);
        console2.log("Chain 6565 balance after bundle send:", sourceBalanceAfter);
        console2.log("Interop fee:", interopFee);
        console2.log("Native balance before interop:", nativeBalanceBefore);

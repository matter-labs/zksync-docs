...
describe('MyNFT', function () {
  ...
  before(async function () {
    ...
    nftContract = await MyNFT.deploy('MyNFTName', 'MNFT', 'https://mybaseuri.com/token/', owner.address);
  ...
  it('Should not allow non-owner to mint NFTs', async function () {
    try {
      const tx3 = await (nftContract.connect(recipient) as Contract).mint(recipient.address);
      await tx3.wait();
